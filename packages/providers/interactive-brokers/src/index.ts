/**
 * @platform/provider-interactive-brokers — the Interactive Brokers broker/venue provider adapter (PLACEHOLDER).
 *
 * Supplies the gateway registry with a Interactive Brokers adapter built on the shared placeholder factory from
...
export const PROVIDER_IDS: readonly ProviderId[] = ['interactive-brokers'];
...
export function createInteractiveBrokersProvider(
  id: ProviderId = 'interactive-brokers',
): BrokerProviderPort {
  if (!PROVIDER_IDS.includes(id))
    throw new Error(`@platform/provider-interactive-brokers does not serve provider "${id}"`);
...
export const providerFactories: Readonly<Record<string, BrokerProviderFactory>> = {
  'interactive-brokers': () => createPlaceholderProvider(describeProvider('interactive-brokers')),
};

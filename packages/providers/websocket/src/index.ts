/**
 * @platform/provider-websocket — the generic WebSocket broker/venue provider adapter (PLACEHOLDER).
 *
 * Supplies the gateway registry with a generic WebSocket adapter built on the shared placeholder factory from
...
export const PROVIDER_IDS: readonly ProviderId[] = ['websocket'];
...
export function createWebsocketProvider(id: ProviderId = 'websocket'): BrokerProviderPort {
  if (!PROVIDER_IDS.includes(id))
    throw new Error(`@platform/provider-websocket does not serve provider "${id}"`);
...
export const providerFactories: Readonly<Record<string, BrokerProviderFactory>> = {
  websocket: () => createPlaceholderProvider(describeProvider('websocket')),
};

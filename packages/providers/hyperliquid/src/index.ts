/**
 * @platform/provider-hyperliquid — the Hyperliquid broker/venue provider adapter (PLACEHOLDER).
 *
 * Supplies the gateway registry with a Hyperliquid adapter built on the shared placeholder factory from
...
export const PROVIDER_IDS: readonly ProviderId[] = ['hyperliquid'];
...
export function createHyperliquidProvider(id: ProviderId = 'hyperliquid'): BrokerProviderPort {
  if (!PROVIDER_IDS.includes(id))
    throw new Error(`@platform/provider-hyperliquid does not serve provider "${id}"`);
...
export const providerFactories: Readonly<Record<string, BrokerProviderFactory>> = {
  hyperliquid: () => createPlaceholderProvider(describeProvider('hyperliquid')),
};

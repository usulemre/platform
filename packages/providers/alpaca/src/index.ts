/**
 * @platform/provider-alpaca — the Alpaca broker/venue provider adapter (PLACEHOLDER).
 *
 * Supplies the gateway registry with a Alpaca adapter built on the shared placeholder factory from
...
export const PROVIDER_IDS: readonly ProviderId[] = ['alpaca'];
...
export function createAlpacaProvider(id: ProviderId = 'alpaca'): BrokerProviderPort {
  if (!PROVIDER_IDS.includes(id))
    throw new Error(`@platform/provider-alpaca does not serve provider "${id}"`);
...
export const providerFactories: Readonly<Record<string, BrokerProviderFactory>> = {
  alpaca: () => createPlaceholderProvider(describeProvider('alpaca')),
};

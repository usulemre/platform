/**
 * @platform/provider-rest — the generic REST broker/venue provider adapter (PLACEHOLDER).
 *
 * Supplies the gateway registry with a generic REST adapter built on the shared placeholder factory from
...
export const PROVIDER_IDS: readonly ProviderId[] = ['rest'];
...
export function createRestProvider(id: ProviderId = 'rest'): BrokerProviderPort {
  if (!PROVIDER_IDS.includes(id))
    throw new Error(`@platform/provider-rest does not serve provider "${id}"`);
...
export const providerFactories: Readonly<Record<string, BrokerProviderFactory>> = {
  rest: () => createPlaceholderProvider(describeProvider('rest')),
};

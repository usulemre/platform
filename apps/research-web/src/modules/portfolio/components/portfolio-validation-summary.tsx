...
/**
 * Portfolio validation summary — surfaces the Validation Foundation verdict.
 * Presentation only; the console never asserts significance (AI-2).
 */
export function PortfolioValidationSummary({ validation }: { validation: ValidationVm }) {
  return (
    <Card>
      <CardHeader className="flex-row items-center justify-between space-y-0">
        <CardTitle>Validation</CardTitle>
        <PortfolioStatusBadge label={validation.label} tone={validation.tone} />
      </CardHeader>
          ...
          <ul className="space-y-1">
            {validation.issues.map((issue, index) => (
              <li key={`${issue.code}-${index}`} className="flex items-start gap-2">
                <PortfolioStatusBadge label={issue.severity} tone={issue.tone} />
                <span>

...
import { useStrategySummary } from '../hooks/use-strategies';
...
/** Strategy dashboard — advisory notice, headline counts and status distribution.
 *  Aggregation is computed by the application service (pure), not here. */
export function StrategyDashboard() {
  const { data, isLoading } = useStrategySummary();
            ...
            <div className="flex flex-wrap items-center gap-2">
              {data.byStatus.map((bucket) => (
                <span key={bucket.value} className="inline-flex items-center gap-1">
                  <StrategyStatusBadge label={bucket.label} tone={bucket.tone} />

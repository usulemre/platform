...
/**
 * Strategy validation summary — surfaces the Validation Foundation verdict.
 * Presentation only; the console never asserts significance (AI-2).
 */
export function StrategyValidationSummary({ validation }: { validation: ValidationVm }) {
  return (
    <Card>
      <CardHeader className="flex-row items-center justify-between space-y-0">
        <CardTitle>Validation</CardTitle>
        <StrategyStatusBadge label={validation.label} tone={validation.tone} />
      </CardHeader>
          ...
          <ul className="space-y-1">
            {validation.issues.map((issue, index) => (
              <li key={`${issue.code}-${index}`} className="flex items-start gap-2">
                <StrategyStatusBadge label={issue.severity} tone={issue.tone} />
                <span>

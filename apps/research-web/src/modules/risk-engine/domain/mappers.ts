/**
 * DTO → view-model mappings + summary/comparison aggregation for the researcher
 * (research-web) Risk Engine module. All presentation and aggregation decisions live
 * here so UI components stay logic-free. Pure and deterministic. Stage labels/ordering,
 ...
 * through unchanged — never computed; comparison is a pure lookup/reshape.
 */
import {
  RISK_STAGES,
  assessmentKey,
...
function toRuleVm(rule: RiskRule): RuleVm {
  return {
    id: rule.id,
    code: rule.code,
    label: rule.label,
...
function toLimitVm(limit: RiskLimit): LimitVm {
  return {
    id: limit.id,
...
function subjectHref(assessment: RiskAssessment): string | undefined {
  if (assessment.subjectKind === 'PORTFOLIO')
    return `/portfolio-construction/${assessment.subjectRef}`;
  if (assessment.subjectKind === 'SIGNAL') return `/signals/${assessment.subjectRef}`;
  if (assessment.subjectKind === 'STRATEGY') return `/strategies/${assessment.subjectRef}`;
  return undefined;
}
...
export function toDetailVm(assessment: RiskAssessment): RiskDetailVm {
  ...
  if (assessment.portfolioRef)
    links.push({ label: 'Portfolio', href: `/portfolio-construction/${assessment.portfolioRef}` });
  if (assessment.backtestRef)
    links.push({ label: 'Backtest', href: `/backtesting/${assessment.backtestRef}` });

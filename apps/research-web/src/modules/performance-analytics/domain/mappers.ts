/**
 * DTO → view-model mappings + summary/comparison aggregation for the researcher (research-web)
 * Performance Analytics module. All presentation and aggregation decisions live here so UI
...
export function toDetailVm(report: PerformanceReport): ReportDetailVm {
  ...
  return {
    ...
    computation: computationStatus(report.computation),
    subject: {
      kind: subjectStatus(report.subjectKind),
      name: report.subjectName,
      href: `${SUBJECT_ROUTE[report.subjectKind]}/${report.subjectRef}`,
    },

...
import { useExperimentSummary } from '../hooks/use-experiments';
...
/** Experiment dashboard — headline counts and status distribution. Aggregation
 *  is computed by the application service (pure), not here. */
export function ExperimentDashboard() {
  const { data, isLoading } = useExperimentSummary();
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        ...
        <StatCard label="Concluded" value={data.concluded} />
      </div>
      {data.byStatus.length > 0 ? (
        <div className="flex flex-wrap items-center gap-2">
          {data.byStatus.map((bucket) => (
            <span key={bucket.value} className="inline-flex items-center gap-1">
              <ExperimentStatusBadge label={bucket.label} tone={bucket.tone} />

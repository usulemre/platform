...
/**
 * Validation summary — surfaces the Validation Foundation verdict for an
 * experiment. Presentation only; the verdict is produced upstream and this
 * console never asserts significance (AI-2).
 */
export function ExperimentValidationSummary({ validation }: { validation: ValidationVm }) {
  return (
    <Card>
      <CardHeader className="flex-row items-center justify-between space-y-0">
        <CardTitle>Validation</CardTitle>
        <ExperimentStatusBadge label={validation.label} tone={validation.tone} />
      </CardHeader>
          ...
          <ul className="space-y-1">
            {validation.issues.map((issue, index) => (
              <li key={`${issue.code}-${index}`} className="flex items-start gap-2">
                <ExperimentStatusBadge label={issue.severity} tone={issue.tone} />
                <span>

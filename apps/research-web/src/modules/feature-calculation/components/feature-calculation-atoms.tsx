...
export function CalcLoading({ rows = 5 }: { rows?: number }) {
  return (
...
export function CalcEmpty({ label }: { label: string }) {
  return <p className="py-4 text-sm text-muted-foreground">{label}</p>;
}

export function CalcError({ onRetry }: { onRetry?: () => void }) {
  return (
    <Alert variant="destructive" title="Unable to load the feature calculation engine">
      <div className="space-y-2">
        <p>
          The feature calculation engine could not be reached. This is expected until the backend is
          available.
        </p>
        {onRetry ? (
...
/** Engine note: this engine performs REAL, deterministic, point-in-time calculations. */
export function EngineNotice() {
  return (
    ...
    >
      ...
      <p>
        The Feature Calculation Engine performs{' '}
        <span className="font-medium text-foreground">real</span>, deterministic, point-in-time
        calculations (SMA, EMA, RSI, MACD, Bollinger Bands, VWAP, …) over canonical datasets. Every
        result is validated for determinism and causality (no look-ahead); values shown here are
        computed live by the calculation library.
      </p>
    </div>
  );
}

...
export function ResearchLoading({ rows = 5 }: { rows?: number }) {
  return (
...
export function ResearchEmpty({ label }: { label: string }) {
  return <p className="py-4 text-sm text-muted-foreground">{label}</p>;
}

export function ResearchError({ onRetry }: { onRetry?: () => void }) {
  return (
    <Alert variant="destructive" title="Unable to load research">
      <div className="space-y-2">
        <p>
          The research engine could not be reached. This is expected until the backend is available.
        </p>
        {onRetry ? (
...
/** Governance note: the engine orchestrates; it never decides significance or
 *  approves capital. Those are deterministic/human governance decisions. */
export function GovernanceNotice() {
  return (
    ...
    >
      ...
      <p>
        The Research Engine <span className="font-medium text-foreground">orchestrates</span> the
        research lifecycle; it never validates significance and never approves capital. Validation
        verdicts and approvals are made by deterministic engines and accountable humans.
      </p>
    </div>
  );
}

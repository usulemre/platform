/**
 * In-memory mock adapter for the Feature Store UI. Synthetic feature METADATA
 * ONLY — no feature calculations, no statistics, no persistence. Dataset/feature
 * references use the other research-web modules' ids so cross-links resolve. This
...
const FEATURES: readonly RegisteredFeature[] = [
  ...
  },
  {
    ...
    lineage: {
      nodes: [{ id: 'N-1', kind: 'FEATURE', ref: 'feat-seasonal', label: 'Seasonal encoding' }],
      edges: [],
    },
...
const FAMILIES: readonly FeatureFamily[] = [
  {
    namespace: 'equities',
    family: 'momentum',
...
export class MockFeatureStoreRepository implements FeatureStoreRepository {
  ...
  async listFeatures(query: FeatureQuery): Promise<readonly RegisteredFeature[]> {
    await this.delay();
    return applyFeatureQuery(FEATURES, query);
  }
  ...
  async listFamilies(): Promise<readonly FeatureFamily[]> {
    await this.delay();
    return FAMILIES;
  }
...
export const FEATURE_STORE_SEED = { features: FEATURES, families: FAMILIES } as const;

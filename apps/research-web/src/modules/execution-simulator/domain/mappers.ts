 ...
 * (research-web) Execution Simulator module. All presentation and aggregation decisions
...
export function toDetailVm(session: SimulationSession): SessionDetailVm {
  ...
  if (session.portfolioRef)
    links.push({ label: 'Portfolio', href: `/portfolio-construction/${session.portfolioRef}` });
  if (session.strategyRef)
    links.push({ label: 'Strategy', href: `/strategies/${session.strategyRef}` });
  if (session.backtestRef)
    links.push({ label: 'Backtest', href: `/backtesting/${session.backtestRef}` });

...
export function NotificationBadge({ label, tone }: { label: string; tone: Tone }) {
...
export function NotificationLoading({ rows = 6 }: { rows?: number }) {
  return (
  ...
  return (
    <Alert variant="destructive" title="Unable to load notifications">
      <div className="space-y-2">
        <p>
          The notification service could not be reached. This is expected until the backend is
          available.
        </p>
        {onRetry ? (
  ...
  return (
    <nav className="flex items-center justify-between gap-4 text-sm" aria-label="Pagination">
      <span className="text-muted-foreground">
        Page {info.page} of {info.totalPages} · {info.total} notification
        {info.total === 1 ? '' : 's'}
      </span>

/**
 * Synthetic post-trade execution records for the TCA UI mock — fills, benchmark prices and explicit
 * costs. NOT market data and NOT from any venue. NO exchange/broker SDK, NO API keys, NO
 * HTTP/WebSocket/FIX, NO connectivity, no persistence. The TCA analysis applied to this data is REAL
 * (from `@platform/tca-sdk`). This is the UI's own mock, independent of the service tier.
 */
...
function build(spec: SeedSpec): ExecutionInput {
  ...
  return {
    ...
    midAfter: spec.arrival + spec.drift,
    venue: spec.venue,
    mode: spec.mode,
    executedAt: spec.at,
  };

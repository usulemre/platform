...
export function RiskLoading({ rows = 5 }: { rows?: number }) {
  return (
...
export function RiskEmpty({ label }: { label: string }) {
  return <p className="py-4 text-sm text-muted-foreground">{label}</p>;
}

export function RiskError({ onRetry }: { onRetry?: () => void }) {
  return (
    <Alert variant="destructive" title="Unable to load the risk engine">
      <div className="space-y-2">
        <p>
          The risk engine could not be reached. This is expected until the backend is available.
        </p>
        {onRetry ? (
...
/** Governance note: the engine orchestrates risk governance; it never computes VaR,
 *  CVaR, expected shortfall, stress tests or exposures, and never decides a risk
 *  verdict — those are executed/decided elsewhere. */
export function GovernanceNotice() {
  return (
    ...
    >
      ...
      <p>
        The Risk Engine <span className="font-medium text-foreground">orchestrates</span> risk
        governance; it never computes VaR, CVaR, expected shortfall, stress tests or exposures, and
        never decides a risk verdict. Exposures come from the risk model and verdicts are made by
        deterministic engines and accountable humans — reflected here, never decided here.
      </p>
    </div>
  );
}

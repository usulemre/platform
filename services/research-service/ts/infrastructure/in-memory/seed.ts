/**
 * Synthetic in-memory seed for development and tests. Research METADATA ONLY — no
 * quantitative algorithms, no statistics, no persistence. Artifacts reference the
 * other modules' ids so cross-links resolve; they are never embedded.
 */
import {
  RESEARCH_STAGES,
  stageOrder,
...
import type { ModuleArtifactHandle } from '../ports';

/** Build a stage-progress list where everything before `current` is COMPLETE,
 *  `current` is IN_PROGRESS (or BLOCKED), and the rest PENDING. */
...
export const TEMPLATES: readonly ResearchTemplate[] = [
  {
    id: 'TPL-FULL',
...
export const MODULE_ARTIFACTS: readonly ModuleArtifactHandle[] = [
  { kind: 'DATASET', ref: 'ds-equity-eod', name: 'US Equity Prices (EOD)' },
  { kind: 'DATASET', ref: 'ds-fx-spot', name: 'FX Spot Rates' },
  { kind: 'FEATURE', ref: 'feat-resid-return', name: 'Residual return (1d)' },
  { kind: 'FEATURE', ref: 'feat-carry', name: 'Carry signal' },
  { kind: 'SIGNAL', ref: 'sig-reversal', name: 'Reversal signal' },
  { kind: 'SIGNAL', ref: 'sig-fx-carry', name: 'FX carry signal' },
  { kind: 'STRATEGY', ref: 'str-reversal-ls', name: 'Reversal long/short' },
  { kind: 'PORTFOLIO', ref: 'port-core', name: 'Core multi-strategy' },
  { kind: 'EXPERIMENT', ref: 'exp-momentum-reversal', name: 'Short-horizon reversal' },
];
